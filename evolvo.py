...
class GFSLExpressionBuilder:
    ...
    def choose(
        ...
        if value_int not in valid:
            raise ValueError(
                f"Invalid option {value_int} for {slot_name(slot_index)}."
            )
        if require_viable:
    ...
    def target(self, category: Category,
               dtype: Optional[Union[DataType, int]] = None,
               index: Optional[int] = None) -> 'GFSLExpressionBuilder':
    ...
    def build(self, *, auto_fill: bool = False,
              max_branching: int = DEFAULT_PROBABILITY_BRANCHING) -> GFSLInstruction:
        if auto_fill:
    ...
    def commit(self, *, auto_fill: bool = False,
               max_branching: int = DEFAULT_PROBABILITY_BRANCHING) -> GFSLInstruction:
    ...
    def _set_pointer(self, cat_slot: int, spec_slot: int,
                     category: Category, spec: int) -> 'GFSLExpressionBuilder':
    ...
    def _coerce_slot_value(self, slot_index: int, value: Any) -> int:
        ...
        if isinstance(value, tuple) and slot_index in (
                SLOT_TARGET_SPEC, SLOT_SOURCE1_SPEC, SLOT_SOURCE2_SPEC
        ):
            if len(value) != 2:
    ...
    def _matches_conditions(self, instruction: GFSLInstruction,
                            conditions: Dict[int, int]) -> bool:
